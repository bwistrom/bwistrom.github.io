class FuturisticWorkspace {
    ...
    init() {
        ...
        // Create new grid platforms
        this.createNewGridPlatform();
        this.createOriginStyleGridPlatform();

        // Create grid platform element
        this.createGridPlatformElement();

        // Create realistic star
        this.createRealisticStar();
    }
    ...
    setupInteraction() {
        ...
        addWindowButton.className = 'add-window';
        addWindowButton.innerHTML = '➕';
        addWindowButton.title = 'Add Window';
        addWindowButton.style.cssText = `
            position: fixed;
            top: 20px;
            left: 20px;
        ...
        addWindowButton.addEventListener('mouseover', () => {
            addWindowButton.style.background = 'rgba(0, 255, 255, 0.1)';
            addWindowButton.style.borderColor = 'rgba(0, 255, 255, 0.8)';
        });

        addWindowButton.addEventListener('mouseout', () => {
            addWindowButton.style.background = 'rgba(0, 0, 0, 0.7)';
            addWindowButton.style.borderColor = 'rgba(0, 255, 255, 0.3)';
        });
        ...
        document.body.appendChild(addWindowButton);
    ...
    createWorkspaceWindow() {
        ...
        // Create window content (holographic display)
        const contentGeometry = new THREE.PlaneGeometry(1.8, 1.8);
        ...
        // Add glow effect
        const glowGeometry = new THREE.BoxGeometry(2.2, 2.2, 0.2);
        ...
        // Position the window
        windowGroup.position.set(0, 0, -5);

        // Add click interaction
        windowGroup.userData.isClickable = true;
        windowGroup.userData.isWorkspaceWindow = true;

        // Add to scene
        this.scene.add(windowGroup);

        // Add to clickable objects
        this.clickableObjects.push(windowGroup);

        // Store reference for animation
        this.windows.push(windowGroup);
    ...
    createRealisticStar() {
        ...
        // Star parameters (based on typical main sequence star)
        const starRadius = 5; // Scaled radius for visualization
        const glowRadius = 15; // Glow effect radius
        const coronaRadius = 25; // Corona effect radius
        ...
        const coronaMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    vec3 finalColor = color * intensity * (0.5 + noise * 0.5) * pulse;
        ...
        // Position the star at a suitable distance (scaled for visualization)
        // Using a distance of 100 units (scaled from typical star distances)
        starGroup.position.set(100, 0, 0);
    ...
    animate() {
        ...
        // Update star animation if it exists
        if (this.starMaterials) {
            const time = Date.now() * 0.001;
            this.starMaterials.glow.uniforms.time.value = time;
            this.starMaterials.corona.uniforms.time.value = time;
        }

        // Update grid platform element animation if it exists
        if (this.gridPlatformDisplayMaterial) {
            this.gridPlatformDisplayMaterial.uniforms.time.value = Date.now() * 0.001;
        }
    }
    ...
    createNewGridPlatform() {
        ...
        const gridHelper = new THREE.GridHelper(gridSize, gridDivisions, 0x00ffc8, 0x00ffc8);
        gridHelper.position.y = 0;
        gridGroup.add(gridHelper);

        // Create platform surface
        const platformGeometry = new THREE.BoxGeometry(gridSize, 0.5, gridSize);
        ...
        platform.userData.surfaceType = 'grid';
        platform.userData.surfaceSize = gridSize;
        platform.userData.surfaceDivisions = gridDivisions;

        // Add glow effect
        const glowGeometry = new THREE.BoxGeometry(gridSize + 2, 0.1, gridSize + 2);
        const glowMaterial = new THREE.MeshBasicMaterial({
            color: 0x00ffc8,
            transparent: true,
            opacity: 0.2
        ...
        // Set position at specified coordinates
        gridGroup.position.set(-496.33, 175.19, 334.98);

        // Add click interaction
        gridGroup.userData.isClickable = true;
        gridGroup.userData.isGridPlatform = true;
        gridGroup.userData.isSurface = true;

        // Add to scene
        this.scene.add(gridGroup);

        // Add to clickable objects
        this.clickableObjects.push(gridGroup);

        return gridGroup;
    }

    createOriginStyleGridPlatform() {
        // Create a new grid group
        const gridGroup = new THREE.Group();
        
        // Create grid lines with origin workspace style
        const gridSize = 50;
        ...
        const glowGeometry = new THREE.BoxGeometry(gridSize + 4, 0.1, gridSize + 4);
        const glowMaterial = new THREE.MeshBasicMaterial({
            color: 0x00ffc8,
            transparent: true,
            opacity: 0.15
        ...
        // Add edge highlights
        const edgeGeometry = new THREE.BoxGeometry(gridSize + 0.2, 0.6, gridSize + 0.2);
        ...
        // Set position at specified coordinates
        gridGroup.position.set(-551.12, -59.46, 353.03);
    ...
    handleWorkspaceClick(clickedObject) {
        if (clickedObject.userData.isGridPlatform || clickedObject.userData.isSurface) {
    ...
    createGridPlatformElement() {
        ...
        const base = new THREE.Mesh(baseGeometry, baseMaterial);
        elementGroup.add(base);

        // Create central column
        const columnGeometry = new THREE.CylinderGeometry(0.2, 0.2, 3, 32);
        ...
        // Add glow effects
        const glowGeometry = new THREE.BoxGeometry(2.2, 4, 2.2);
        const glowMaterial = new THREE.MeshBasicMaterial({
            color: 0x00ffc8,
            transparent: true,
            opacity: 0.1
        ...
        const glow = new THREE.Mesh(glowGeometry, glowMaterial);
        glow.position.y = 1.5;
        elementGroup.add(glow);

        // Add holographic display
        const displayGeometry = new THREE.PlaneGeometry(1.5, 1.5);
        const displayMaterial = new THREE.ShaderMaterial({
            uniforms: {
                time: { value: 0 },
                color: { value: new THREE.Color(0x00ffc8) }
        ...
        // Create four displays around the column
        const displayPositions = [
            { x: 1, y: 2, z: 0, rotation: 0 },
            { x: -1, y: 2, z: 0, rotation: Math.PI },
            { x: 0, y: 2, z: 1, rotation: Math.PI / 2 },
            { x: 0, y: 2, z: -1, rotation: -Math.PI / 2 }
        ];
        ...
        // Store reference for animation
        this.gridPlatformElement = elementGroup;
        this.gridPlatformDisplayMaterial = displayMaterial;
