class FuturisticWorkspace {
    ...
    createEnvironment() {
        ...
        // Create Mirrored Dimension
        this.createMirroredDimension();
    }
    ...
    triggerPortalReload() {
        ...
        // Randomize dimension count for mirrored dimension
        if (Math.random() < 0.5) {
            this.dimensionCount = Math.floor(Math.random() * 10) + 1;
            this.createMirroredDimension();
        }
        
        // Update globe materials with new dimension count
        if (this.blueGlobe) {
            this.blueGlobe.traverse(child => {
                if (child.material && child.material.uniforms) {
                    child.material.uniforms.dimensionCount.value = this.dimensionCount;
                }
            });
        }

        // Update mirrored dimension if it exists
        if (this.mirroredDimension) {
            this.mirroredDimension.traverse(child => {
                if (child.material && child.material.uniforms) {
                    child.material.uniforms.dimensionCount.value = this.dimensionCount;
                }
            });
        ...
        // Add dimension count to message
        reloadContainer.innerHTML = `Entering Dimension ${this.dimensionCount}...`;
    ...
    createWorkspaceWindow() {
        ...
        // Get current time from counter
        const currentTime = document.getElementById('timeCounter').textContent;
        const [hours, minutes, seconds] = currentTime.split(':').map(Number);
        
        // Calculate position based on time
        const timeInSeconds = hours * 3600 + minutes * 60 + seconds;
        const windowIndex = Math.floor(timeInSeconds / 11); // New window every 11 seconds
        
        // Position windows in a pattern
        const radius = 3;
        const angle = (windowIndex * Math.PI * 2) / 12; // 12 windows in a circle
        const x = Math.cos(angle) * radius;
        const y = 2; // Fixed height
        const z = Math.sin(angle) * radius;
        
        window.position.set(x, y, z);
        
        // Add gravitational pull properties
        window.userData = {
            creationTime: timeInSeconds,
            gravitationalPull: 0.02,
            isActive: false
        };
        ...
        // Check for window grouping
        this.checkWindowGrouping();
    }
    ...
    checkWindowGrouping() {
        ...
        // Check if windows are aligned (every minute)
    ...
    activateWindowGroup() {
        ...
        const gravitationalField = document.createElement('div');
        gravitationalField.style.cssText = `
            position: fixed;
            top: 50%;
            left: 50%;
            transform: translate(-50%, -50%);
            background: radial-gradient(circle at center, rgba(0, 255, 200, 0.3) 0%, rgba(0, 255, 200, 0) 70%);
            width: 100vw;
            height: 100vh;
            pointer-events: none;
            z-index: 998;
            ...
            transition: opacity 0.5s ease;
        `;
        document.body.appendChild(gravitationalField);
        
        // Fade in
        setTimeout(() => {
            gravitationalField.style.opacity = '1';
        }, 10);
        
        // Activate all windows
        this.windows.forEach(window => {
            window.userData.isActive = true;
        });
        
        // Show message
        this.showHomeMessage();
        
        // Remove gravitational field after 5 seconds
        setTimeout(() => {
            gravitationalField.style.opacity = '0';
            setTimeout(() => {
                gravitationalField.remove();
            }, 500);
        }, 5000);
    }
    ...
    showHomeMessage() {
        ...
        messageContainer.innerHTML = `
            <h2 style="margin-bottom: 1rem; font-size: 2rem;">Great Women always find the way back home</h2>
            <p style="margin-bottom: 1.5rem;">The path to Stellarium awaits...</p>
            <button id="activateStellarium" style="
                background: #00ffc8;
                color: #0a0a0f;
                border: none;
                padding: 0.5rem 1rem;
                ...
                font-family: 'Orbitron', sans-serif;
                transition: all 0.3s ease;
            ">Activate Stellarium Portal</button>
        `;
        
        document.body.appendChild(messageContainer);
        
        // Add button event listener
        document.getElementById('activateStellarium').addEventListener('click', () => {
            window.open('https://stellarium.org/releases/software/', '_blank');
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    if (dimensionCount >= 4.0) {
                        // RGB color cycling for dimension 4+
                        float r = sin(time * 0.5) * 0.5 + 0.5;
                        float g = sin(time * 0.5 + 2.094) * 0.5 + 0.5;
                        float b = sin(time * 0.5 + 4.188) * 0.5 + 0.5;
                        finalColor = vec3(r, g, b);
                    } else {
                        finalColor = color * (0.5 + pulse * 0.5);
                    }
    ...
    createMirroredDimension() {
        // Create a new dimension with a pentagon globe
        const pentagonGeometry = new THREE.CylinderGeometry(1, 1, 0.1, 5);
        const pentagonMaterial = new THREE.ShaderMaterial({
            uniforms: {
                time: { value: 0 },
                color: { value: new THREE.Color(0xff00ff) },
                dimensionCount: { value: this.dimensionCount }
            },
            vertexShader: `
                varying vec2 vUv;
                varying vec3 vPosition;
        ...
        const pentagonGlobe = new THREE.Mesh(pentagonGeometry, pentagonMaterial);
        pentagonGlobe.position.set(-5, 2, 5); // Position opposite to blue globe

        // Add "MW" text to the pentagon
        const textGeometry = new THREE.TextGeometry('MW', {
            font: this.font,
            size: 0.5,
            height: 0.1,
            curveSegments: 12,
        ...
        pentagonGlobe.add(textMesh);

        // Add energy rings
        for (let i = 0; i < 3; i++) {
            const ringGeometry = new THREE.RingGeometry(1.2 + i * 0.3, 1.3 + i * 0.3, 5);
            const ringMaterial = new THREE.ShaderMaterial({
                uniforms: {
                    time: { value: 0 },
                    color: { value: new THREE.Color(0xff00ff) },
            ...
            const ring = new THREE.Mesh(ringGeometry, ringMaterial);
            ring.rotation.x = Math.PI / 2;
            pentagonGlobe.add(ring);
        }

        // Add gravitational pull properties
        pentagonGlobe.userData = {
            gravitationalPull: 0.02,
            isActive: false,
        ...
        // Add to scene
        this.scene.add(pentagonGlobe);
        this.pentagonGlobe = pentagonGlobe;
    }
    ...
    animate() {
        ...
        // Update windows with gravitational pull
        this.windows.forEach(window => {
            if (window.userData.isActive) {
                const direction = this.portal.position.clone().sub(window.position);
                const distance = direction.length();
                const pullStrength = Math.min(1, (5 - distance) / 5);
                window.position.add(direction.normalize().multiplyScalar(window.userData.gravitationalPull * pullStrength));
            }
        });

        // Update mirrored dimension
        if (this.mirroredDimension) {
            this.mirroredDimension.traverse(child => {
                if (child.material && child.material.uniforms) {
                    child.material.uniforms.time.value = time;
            ...
            this.mirroredDimension.rotation.y = -time * 0.2; // Rotate in opposite direction
        }

        // Update pentagon globe with gravitational pull
        if (this.pentagonGlobe && this.blueGlobe) {
            const pentagon = this.pentagonGlobe;
            const blueGlobe = this.blueGlobe;
            
            // Calculate distance between globes
            const distance = pentagon.position.distanceTo(blueGlobe.position);
            
            // Activate gravitational pull when within 10 units
            if (distance < 10) {
                pentagon.userData.isActive = true;
                
                // Calculate direction to blue globe
                const direction = blueGlobe.position.clone().sub(pentagon.position);
                const pullStrength = Math.min(1, (10 - distance) / 10);
                
                // Apply gravitational pull
                pentagon.position.add(direction.normalize().multiplyScalar(pentagon.userData.gravitationalPull * pullStrength));
                
                // Check if passed through blue globe
                if (distance < 1 && !pentagon.userData.hasPassedThrough) {
                    pentagon.userData.hasPassedThrough = true;
                    
                    // Create visual effect for passing through
                    ...
                    const effectMaterial = new THREE.ShaderMaterial({
                        ...
                        fragmentShader: `
                            ...
                            void main() {
                                float pulse = sin(time * 4.0 + vUv.x * 20.0) * 0.5 + 0.5;
                                vec3 finalColor = color * (0.5 + pulse * 0.5);
                                float alpha = 0.6 + pulse * 0.4;
                                gl_FragColor = vec4(finalColor, alpha);
                            }
                    ...
                    effect.position.copy(blueGlobe.position);
                    ...
                    const animateEffect = () => {
                        ...
                        if (elapsed < 2) {
                            effect.scale.setScalar(1 + elapsed);
                            effect.material.uniforms.time.value = time;
                            effect.material.opacity = 1 - elapsed / 2;
                ...
                pentagon.userData.isActive = false;
                pentagon.userData.hasPassedThrough = false;
            }
            
            // Update pentagon materials
            pentagon.traverse(child => {
                if (child.material && child.material.uniforms) {
                    child.material.uniforms.time.value = time;
                    child.material.uniforms.dimensionCount.value = this.dimensionCount;
                }
            ...
            // Rotate pentagon in opposite direction of blue globe
            pentagon.rotation.y = -time * 0.2;
        }
    }
