class FuturisticWorkspace {
    constructor() {
        ...
        this.fpsElement.style.color = '#00ffc8';
        ...
        this.renderer.setPixelRatio(1); // Force pixel ratio to 1 for better performance
        ...
        this.renderer.info.autoReset = false; // Prevent automatic info reset
    ...
    createPlatformGlobe(platformPosition, index) {
        ...
        const globeMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Smoother gravitational effect with resonance
                    float gravityEffect = 1.0 - smoothstep(0.0, 1.5, dist);
                    float resonanceEffect = sin(dist * 10.0 + time * resonance) * 0.5 + 0.5;
                    float timeFreeze = smoothstep(0.3, 0.7, gravityEffect * resonanceEffect);
                    vec3 finalColor = mix(color, vec3(1.0), timeFreeze);
                    float alpha = 0.8 + timeFreeze * 0.2;
        ...
        // Add energy rings with harmonic resonance
        for (let i = 0; i < 3; i++) {
            ...
            const ringMaterial = new THREE.ShaderMaterial({
                ...
                fragmentShader: `
                    ...
                    void main() {
                        ...
                        float gravityEffect = 1.0 - smoothstep(0.0, 1.5, dist);
                        float resonanceEffect = sin(angle * 10.0 + time * resonance) * 0.5 + 0.5;
                        float timeFreeze = smoothstep(0.3, 0.7, gravityEffect * resonanceEffect);
                        vec3 finalColor = mix(color, vec3(1.0), timeFreeze);
                        float alpha = 0.3 + timeFreeze * 0.2;
                        gl_FragColor = vec4(finalColor, alpha);
                    }
                `,
                transparent: true,
        ...
        // Add gravitational field effect with harmonic resonance
        const gravityFieldGeometry = new THREE.SphereGeometry(2.5, 32, 32);
        const gravityFieldMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    float dist = length(vPosition);
                    float field = 1.0 - smoothstep(1.5, 2.5, dist);
                    float resonanceEffect = sin(dist * 5.0 + time * resonance) * 0.5 + 0.5;
                    vec3 finalColor = color * field * resonanceEffect;
                    float alpha = 0.1 * field * resonanceEffect;
                    gl_FragColor = vec4(finalColor, alpha);
                }
            `,
            transparent: true,
    ...
    createGrid() {
        ...
        this.secondGrid = new THREE.GridHelper(size, divisions, 0x00ffc8, 0x004433);
        this.secondGrid.position.set(150, 48.36, -5.90);

        this.thirdGrid = new THREE.GridHelper(size, divisions, 0x00ffc8, 0x004433);
        this.thirdGrid.position.set(-150, 48.36, 5.90);
    }
    ...
    createPortal() {
        ...
        this.portalDistanceTracking = {
            startPosition: new THREE.Vector3(100, 846.70, 388),
    ...
    setupInteraction() {
        ...
        // Add click event listener for workspace windows
        this.renderer.domElement.addEventListener('click', (event) => {
            ...
            if (intersects.length > 0) {
                ...
                if (clickedObject.userData.isWorkspaceWindow) {
                    this.handleWorkspaceClick(clickedObject);
                }
            }
        });
    }
    ...
    createBlueGlobe() {
        // Create the globe sphere with improved geometry
        ...
        const globeMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Enhanced gravity effect with smooth transition
                    float gravityEffect = 1.0 - smoothstep(0.0, 1.5, dist);
                    ...
                    float distortionEffect = mix(1.0, noiseValue, distortion);
                    
                    // Enhanced color blending
                    vec3 baseColor = color;
                    vec3 energyColor = vec3(0.5, 0.8, 1.0);
                    vec3 finalColor = mix(baseColor, energyColor, pulse * pulseIntensity);
                    
                    // Add energy glow
                    float glow = smoothstep(1.0, 0.0, dist) * energyLevel;
                    finalColor += energyColor * glow;
                    
                    // Add surface detail
                    float surfaceDetail = noise(vUv * 20.0);
                    finalColor *= 1.0 + surfaceDetail * 0.2;
                    
                    // Enhanced transparency
                    float alpha = 0.8 + gravityEffect * 0.2 + glow * 0.3;
                    
                    // Add fresnel effect
                    float fresnel = pow(1.0 - max(dot(vNormal, vec3(0.0, 0.0, 1.0)), 0.0), 2.0);
                    finalColor += vec3(0.2, 0.4, 0.8) * fresnel;
        ...
        // Enhanced energy rings with improved visuals
        for (let i = 0; i < 3; i++) {
            ...
            const ringMaterial = new THREE.ShaderMaterial({
                ...
                fragmentShader: `
                    ...
                    void main() {
                        ...
                        float ringPulse = sin(time * (2.0 + vRingIndex) + angle * 5.0) * 0.5 + 0.5;
                        float ringEnergy = smoothstep(0.5, 0.4, dist) * (1.0 + ringPulse * 0.5);
                        
                        // Add energy variation
                        float energyVariation = sin(time * 3.0 + vRingIndex * 2.0) * 0.5 + 0.5;
                        
                        vec3 finalColor = mix(color, vec3(1.0), ringEnergy * energyVariation);
                        float alpha = 0.3 + ringEnergy * 0.4 + energyLevel * 0.3;
        ...
        // Enhanced gravitational field effect
        const gravityFieldGeometry = new THREE.SphereGeometry(2.5, 64, 64);
        const gravityFieldMaterial = new THREE.ShaderMaterial({
            uniforms: {
                time: { value: 0 },
                color: { value: new THREE.Color(0x0088ff) },
                fieldStrength: { value: 0.0 },
                distortion: { value: 0.0 }
            },
            vertexShader: `
                varying vec2 vUv;
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Add field distortion
                    float noiseValue = noise(vUv * 5.0 + time * 0.2);
                    field *= 1.0 + noiseValue * distortion;
                    
                    // Add energy waves
                    float wave = sin(dist * 10.0 - time * 2.0) * 0.5 + 0.5;
                    field *= 1.0 + wave * 0.2;
                    
                    vec3 finalColor = color * field;
                    float alpha = 0.2 * field * (1.0 + fieldStrength * 0.5);
    ...
    createGridMapping() {
        ...
        const DISTANCE_BETWEEN_GRIDS = 100; // Distance in kilometers
    ...
    createEnergyOrb() {
        ...
        // Add energy rings around the orb
        for (let i = 0; i < 3; i++) {
            const ringGeometry = new THREE.RingGeometry(1.2 + i * 0.3, 1.3 + i * 0.3, 64);
            const ringMaterial = new THREE.ShaderMaterial({
                uniforms: {
                ...
                fragmentShader: `
                    ...
                    void main() {
                        ...
                        // Enhanced ring effect with purple energy
                        float ringPulse = sin(time * (2.0 + vRingIndex) + angle * 5.0) * 0.5 + 0.5;
                        float ringEnergy = smoothstep(0.5, 0.4, dist) * (1.0 + ringPulse * 0.5);
                        ...
                        vec3 finalColor = mix(color, vec3(1.0), ringEnergy * energyVariation);
                        float alpha = 0.3 + ringEnergy * 0.4;
        ...
        const fieldGeometry = new THREE.SphereGeometry(2.5, 64, 64);
    ...
    animate() {
        ...
        // Update platform globes with harmonic gravitational fields
        if (this.platformGlobes) {
            this.platformGlobes.forEach((globe, index) => {
                if (globe.material && globe.material.uniforms) {
                    globe.material.uniforms.time.value = time;
                    // Create harmonic resonance between fields
                    const baseFrequency = 0.5;
                    const harmonicRatio = 1.618; // Golden ratio for natural harmony
                    globe.material.uniforms.gravityField.value = Math.sin(time * baseFrequency + index * Math.PI / 2) * 0.3;
                    globe.material.uniforms.resonance.value = Math.sin(time * baseFrequency * harmonicRatio + index * Math.PI / 2) * 0.5;
                }
                
                // Add subtle orbital movement with harmonic resonance
                const baseX = globe.position.x;
                const baseZ = globe.position.z;
                const radius = Math.sqrt(baseX * baseX + baseZ * baseZ);
                const baseAngle = Math.atan2(baseZ, baseX);
                const harmonicAngle = baseAngle + time * 0.1 * (1 + Math.sin(time * 0.5) * 0.2);
                globe.position.x = Math.cos(harmonicAngle) * radius;
                globe.position.z = Math.sin(harmonicAngle) * radius;
            });
        }

        // Optimize updates based on performance
        if (!this.isLowPerformance || this.frameCount % 2 === 0) {
            // Update particle systems
            this.updateParticleSystems(time);
            
            // Update matrix effect
            this.updateMatrixEffect(time);
            
            // Update quantum field
            this.updateQuantumField(time);
            
            // Update neural network
            this.updateNeuralNetwork(time);
        }

        // Update platform windows with reduced frequency in low performance mode
        if (!this.isLowPerformance || this.frameCount % 4 === 0) {
            this.windows.forEach((window, index) => {
                if (window.material && window.material.uniforms) {
                    window.material.uniforms.time.value = time;
                }
            });
        }
        ...
        if (this.blueGlobe) {
            const globeMaterial = this.blueGlobe.material;
            if (globeMaterial.uniforms) {
                globeMaterial.uniforms.time.value = time;
                globeMaterial.uniforms.pulseIntensity.value = this.pulseIntensity;
                globeMaterial.uniforms.energyLevel.value = this.energyLevel;
                globeMaterial.uniforms.distortion.value = this.distortion;
            }

            // Update rings
            this.blueGlobe.children.forEach((child, index) => {
                if (child.material && child.material.uniforms) {
                    child.material.uniforms.time.value = time;
                    child.material.uniforms.energyLevel.value = this.energyLevel;
                }
            });

            // Update gravitational field
            const gravityField = this.blueGlobe.children[this.blueGlobe.children.length - 1];
            if (gravityField.material && gravityField.material.uniforms) {
                gravityField.material.uniforms.time.value = time;
                gravityField.material.uniforms.fieldStrength.value = this.fieldStrength;
                gravityField.material.uniforms.distortion.value = this.distortion;
            }
        }
        ...
        if (this.pyramid) {
            ...
            this.pyramid.rotation.x = Math.sin(time * 0.05) * 0.2;
            this.pyramid.rotation.z = Math.cos(time * 0.05) * 0.2;
        }
    }

    checkPerformance() {
        const newIsLowPerformance = this.fps < 30;
        
        if (newIsLowPerformance !== this.isLowPerformance) {
            this.isLowPerformance = newIsLowPerformance;
    ...
    adjustQualityForPerformance() {
        if (this.isLowPerformance) {
            ...
            this.particleSystems.forEach(system => {
                if (system.geometry) {
                    system.geometry.setDrawRange(0, Math.floor(system.geometry.attributes.position.count * 0.5));
                }
            });
            ...
            this.windows.forEach(window => {
                if (window.material && window.material.uniforms) {
                    window.material.uniforms.time.value *= 0.5;
                }
            });
            ...
            this.particleSystems.forEach(system => {
                if (system.geometry) {
                    system.geometry.setDrawRange(0, system.geometry.attributes.position.count);
                }
            });
            ...
            this.windows.forEach(window => {
                if (window.material && window.material.uniforms) {
                    window.material.uniforms.time.value *= 2;
                }
            });
    ...
    updateParticleSystems(time) {
        if (this.particles) {
            ...
            for (let i = 0; i < positions.length; i += 3) {
                // Store original positions for orbital movement
                ...
                const baseAngle = Math.atan2(originalZ, originalX);
                
                // Add time-based rotation with different speeds based on radius
                const rotationSpeed = 0.0005 * (1 + radius * 0.1); // Faster rotation for outer particles
                const angle = baseAngle + time * rotationSpeed;
                
                // Update positions with orbital movement
                positions[i] = Math.cos(angle) * radius;
                positions[i + 2] = Math.sin(angle) * radius;
                ...
                // Update colors with time-based transitions
                colors[i] = Math.sin(time * 0.2 + i) * 0.5 + 0.5;     // Red
                colors[i + 1] = Math.cos(time * 0.3 + i) * 0.5 + 0.5; // Green
                colors[i + 2] = Math.sin(time * 0.4 + i) * 0.5 + 0.5; // Blue
            }
    ...
    updateMatrixEffect(time) {
        // Update matrix effect based on time
        // ... existing code ...
    }

    updateQuantumField(time) {
        // Update quantum field based on time
        // ... existing code ...
    }

    updateNeuralNetwork(time) {
        // Update neural network based on time
        // ... existing code ...
    }
