class FuturisticWorkspace {
    ...
    createPlatformGlobe(platformPosition, index) {
        ...
        const globeMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    float timeFreeze = step(0.5, gravityEffect);
            ...
            const ringMaterial = new THREE.ShaderMaterial({
                ...
                fragmentShader: `
                    ...
                    void main() {
                        ...
                        float gravityEffect = 1.0 - smoothstep(0.0, 1.5, length(vec2(vUv.x - 0.5, vUv.y - 0.5)));
                        float timeFreeze = step(0.5, gravityEffect);
                        vec3 finalColor = mix(color, vec3(1.0), timeFreeze);
        ...
        // Add gravitational field effect with reduced influence
        ...
        const gravityFieldMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    float alpha = 0.1 * field; // Reduced alpha for less influence
                    gl_FragColor = vec4(finalColor, alpha);
                }
            `,
            transparent: true,
    ...
    animate() {
        ...
        // Update platform globes with reduced gravitational influence
        if (this.platformGlobes) {
            this.platformGlobes.forEach((globe, index) => {
                if (globe.material && globe.material.uniforms) {
                    ...
                    globe.material.uniforms.gravityField.value = Math.sin(time * 0.5 + index) * 0.3; // Reduced gravity field oscillation
                }
                
                // Add subtle orbital movement to globes
                const baseX = globe.position.x;
                const baseZ = globe.position.z;
                const radius = Math.sqrt(baseX * baseX + baseZ * baseZ);
                const angle = Math.atan2(baseZ, baseX) + time * 0.1; // Slower orbital movement
                globe.position.x = Math.cos(angle) * radius;
                globe.position.z = Math.sin(angle) * radius;
            });
        }
